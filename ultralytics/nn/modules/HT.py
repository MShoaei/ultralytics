...
def hough_transform(rows, cols, theta_res, rho_res):
    ...
    coords = np.concatenate((coords_r[:, None], coords_w[:, None]), axis=1).astype(
        np.float32
    )
...
class HT(nn.Module):
    ...
    def forward(self, image):
        ...
        #     self.vote_index = nn.Parameter(
        #         torch.ones(32 * 32, 46 * 60), requires_grad=False
        #     )
...
class IHT(nn.Module):
    ...
    def forward(self, input_HT):
        ...
        #     self.vote_index = nn.Parameter(
        #         torch.ones(32 * 32, 46 * 60).t(), requires_grad=False
        #     )
        #     self.r, self.c, self.h, self.w = 32, 32, 46, 60
...
class HTIHT(nn.Module):
    def __init__(self, vote_index, inplanes, outplanes):
        ...
        self.conv2 = nn.Sequential(
            *make_conv_block(
                inplanes, outplanes, kernel_size=(9, 1), padding=(4, 0), bias=True
            )
        )
        self.conv3 = nn.Sequential(
            *make_conv_block(
                outplanes, outplanes, kernel_size=(9, 1), padding=(4, 0), bias=True
            )
        )
        ...
        nn.init.kaiming_normal_(
            self.conv3[0].weight, mode="fan_out", nonlinearity="relu"
        )
...
class CAT_HTIHT(nn.Module):
    def __init__(
        ...
        self.conv_cat = nn.Conv2d(
            inplanes + outplanes, inplanes, kernel_size=3, padding=1, bias=False
        )
